...
path = 'e:/2A/gh-pages-dist/infographic/index.html'
...
if 'display: none !important' not in text:
    text = text.replace('</head>', css_snippet)
...
keys = sorted(list(db.keys()), key=len, reverse=True)

for vi in keys:
    en = db[vi]
    search_str = f'<span lang="vi">{vi}</span>'
    if search_str not in text:
        text = text.replace(vi, f'<span lang="vi">{vi}</span><span lang="en">{en}</span>')

with io.open(path, 'w', encoding='utf-8') as f:
    f.write(text)

try:
    src_path = 'e:/2A/src/html/infographic.html'
    with io.open(src_path, 'r', encoding='utf-8') as f:
        src = f.read()
    if 'display: none !important' not in src:
        src = src.replace('</head>', css_snippet)
    for vi in keys:
        en = db[vi]
        search_str = f'<span lang="vi">{vi}</span>'
        if search_str not in src:
            src = src.replace(vi, f'<span lang="vi">{vi}</span><span lang="en">{en}</span>')
    with io.open(src_path, 'w', encoding='utf-8') as f:
        f.write(src)
except:
    pass

print('Done html parsing in Python.')