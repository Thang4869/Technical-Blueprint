...
# Ensure we process keys from longest to shortest so substrings don't break HTML tags accidentally
for vi in sorted(db.keys(), key=len, reverse=True):
    en = db[vi]
    # Simple replacement that wraps the original Vietnamese text and the English translation
...
try:
    ...
    with open(file_path_src, 'r', encoding='utf-8') as f:
        src_text = f.read()
    src_text = src_text.replace('</head>', css_snippet)
    for vi in sorted(db.keys(), key=len, reverse=True):
        en = db[vi]
        src_text = src_text.replace(vi, f'<span lang="vi">{vi}</span><span lang="en">{en}</span>')
    with open(file_path_src, 'w', encoding='utf-8') as f:
        f.write(src_text)
except Exception:
    pass
