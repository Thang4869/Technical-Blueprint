...
# sort by longest string to shortest
keys = sorted(list(db.keys()), key=len, reverse=True)

for vi in keys:
    en = db[vi]
    search_str = '<span lang="vi">{}</span>'.format(vi)
    if search_str not in text:
        text = text.replace(vi, '<span lang="vi">{}</span><span lang="en">{}</span>'.format(vi, en))

with io.open(path, 'w', encoding='utf-8') as f:
    f.write(text)

print("done script html string replace")