...
def process(path):
    ...
    # Strip existing nested spans
    content = re.sub(r'<span lang="(?:vi|en)">|<span lang=\'(?:vi|en)\'>|</span>', '', content)
    ...
    db = {
        ...
        'Phân tích DNA của trò chơi để xác định trọng tâm phát triển. Game tập trung tuyệt đối vào nhịp độ và số lượng vật thể thay vì logic AI phức tạp.': 'Analyze the games DNA to identify development focus. The game strictly focuses on pacing and entity count over complex AI logic.',
        ...
        'Sự thành công của game đến từ một chu trình lặp gây nghiện (Feedback Loop) được thiết kế cực kỳ chặt chẽ, lặp lại hàng trăm lần mỗi phiên chơi.': 'The games success stems from an extremely tight, addictive Feedback Loop, repeating hundreds of times per session.',
        ...
        'Mỗi vũ khí có Logic riêng: Kẻ địch gần nhất (Nearest Euclidean), Kẻ địch máu cao nhất, Random hướng, hoặc bắn theo hướng Vector di chuyển của Player.': 'Each weapon has its own Logic: Nearest Euclidean, Highest Health, Random direction, or firing along the Players Movement Vector.',
    ...
    # Helper function to protect HTML tags from being matched inside keys 
    # but for simple strings replace works fine. 
    # Because of previous strip, we just do normal replace.
